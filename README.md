...
---
## Completed tasks
- deploy the app on several VMs running as 1 kubernetes (k8s) control plane node and 2 worker nodes
...
###############################################
###On k8s-main VM as vagrant user:
###############################################
...
###############################################
###On all k8s VMs as vagrant user:
###############################################   
...
% docker build -t dleung331d/my-webpage .
    For example, docker build -t dleung331d/my-webpage .
% docker login
% docker push dleung331d/my-webpage
